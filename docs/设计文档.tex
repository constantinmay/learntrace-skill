...
\section*{作品速览}
\addcontentsline{toc}{section}{作品速览}

\begin{itemize}
  \item \textbf{面向问题：} AI 辅助课程项目结束后，学生难以说明自己的决策、验证和学习过程。
  \item \textbf{核心方案：} 将 Git、项目文档、已有测试日志和经授权的 AI 轨迹转换为带来源的学习证据。
  \item \textbf{交互方式：} 提供 CLI、宿主 Agent 和本地 Web 工作区三种入口。
  \item \textbf{核心产物：} 可编辑 Markdown 学习档案、待确认问题清单和机器可审计 JSON 记录。
  \item \textbf{关键创新：} 事实、系统候选和学生确认分层保存，不凭时间邻近为 AI 轨迹与 Git 提交建立因果关系。
\end{itemize}

\noindent\textbf{一句话价值：} LearnTrace 不替学生编造成长故事，而是帮助学生用更少、更清楚、可回查的证据说明自己的学习。

\section{作品实现与验收结果}

作品已经形成从材料发现、授权、静态解析、候选生成、学生确认到学习档案导出的完整闭环。评审既可检查仓库中的脱敏合成案例，也可安装 wheel 后运行 \texttt{learntrace ui <project-dir>} 进行本地验收。
...
验收项目 & 实际结果 \\
\midrule
静态证据解析 & 从 Git、文档和已有测试日志生成带来源引用的统一事实 \\
轨迹适配 & 支持逐文件授权的 OpenCode、Claude Code 和 Codex 导出 \\
学习档案输出 & 生成工作版/提交版 Markdown 档案、问题清单和 JSON 归档 \\
Web 工作区 & 支持模型配置、授权、对话、结构化问题卡片、停止和报告阅读 \\
安全边界 & 未授权轨迹不读取；解析器不执行项目代码、测试或日志命令 \\
可复查案例 & \texttt{examples/task4-demo} 随附设计约束、失败日志、补丁和合成轨迹 \\
跨平台交付 & Ubuntu 与 Windows CI 均通过，wheel 包含 Web、Agent 和 Skill 资源 \\
\bottomrule
\end{tabularx}
\end{center}

\noindent\textbf{验收入口：} 图形路径运行 \texttt{learntrace ui <project-dir>}；本地确定性路径运行 \texttt{learntrace discover <project-dir>} 和 \texttt{learntrace run <project-dir>}；五步证据闭环见本文“可复查案例设计”。
...
\subsection{在 Claude Code 中直接使用}

LearnTrace 可以直接安装为 Claude Code Skill，不需要经过自带 Web 工作区。首先确保 \texttt{learntrace} CLI 已安装并在 \texttt{PATH} 中，再将完整的 \texttt{skills/learntrace/} 目录（包括 \texttt{references/}）复制到项目级 \texttt{.claude/skills/learntrace/}，或用户级 \texttt{\textasciitilde/.claude/skills/learntrace/}。项目级安装只对当前项目生效，用户级安装可供本机所有 Claude Code 项目发现。首次创建 Skill 根目录后若未立即识别，重启一次 Claude Code 即可。
...
Claude Code 提供模型连接、对话和工具审批，LearnTrace CLI 承担本地材料发现、解析、脱敏、候选生成、契约校验和档案落盘。两者之间没有第二套远程模型调用，因此仅使用宿主 Skill 时无需为 LearnTrace CLI 配置 API Key，也无需安装 Node.js。

需要特别区分“运行 Skill”和“导入 Claude Code 历史会话”。调用 \texttt{/learntrace} 不会授权系统读取历史目录；只有在学生希望把 AI 协作过程纳入档案时，才需要自行复制指定会话 JSONL，并通过 \texttt{--claude-code-export} 和 \texttt{--authorize-claude-code-export} 对同一文件显式授权。未提供或未授权时，系统仍可仅依据 Git、项目文档和已有测试日志生成档案。

OpenCode 的安装逻辑相同，但使用项目级 \texttt{.opencode/skills/learntrace/} 或用户级 \texttt{\textasciitilde/.config/opencode/skills/learntrace/}，并通过 \texttt{\$learntrace} 调用。宿主始终负责模型登录、会话权限和最终工具审批；Skill 是可审阅的工作流约束，不替代宿主权限系统或操作系统沙箱。

\subsection{学生在项目开发期间的注意事项}

LearnTrace 支持在项目阶段结束后直接整理既有材料，不要求学生为了系统改变正常开发流程。若学生从项目开始便保持真实、自然的工程记录，最终档案会更完整，证据也更容易复核。这里的原则是“保留已经发生的过程”，而不是为了获得更多候选节点制造行为记录。

\begin{enumerate}
  \item \textbf{尽早建立版本历史。} 项目开始时初始化 Git，在需求完成、缺陷修复、方案调整等真实阶段边界提交。不要在项目结束后一次性补交，也不要为了生成学习档案伪造提交、时间或作者信息。
  \item \textbf{让提交能够说明实际变化。} 提交信息简要描述完成或修复的对象，例如“修复 CSV 空行解析”或“补充权限边界测试”，避免只写“更新”“修改”。提交信息属于导航线索，不被系统直接解释为学生动机或学习成果。
  \item \textbf{保留需求和设计变化。} 保存任务书、需求说明、设计文档和必要的阶段记录。约束改变时记录改变了什么；为什么改变、从中学到了什么，仍由学生在确认环节本人说明。
  \item \textbf{保存真实测试日志。} 重要测试应留下执行时间、测试工具、通过或失败状态及必要的用例信息，尤其保留“失败—修复—再次验证”的连续证据。LearnTrace 不自动执行目标项目，也不会在缺少日志时宣称验证成功。
  \item \textbf{在关键节点做轻量复盘。} 遇到重要决策、失败修复或方案调整时，可简要记录问题、选择理由和验证方式。不必记录每次编辑，更不能事后补写并不存在的思考过程。
  \item \textbf{有选择地保留 AI 协作记录。} 只保留确有学习价值且愿意用于复盘的会话。导入时由学生选择文件并逐份授权；API Key、私人聊天、原始会话文件和未经检查的工具输出不得提交到仓库或直接对外分享。
  \item \textbf{提前界定团队项目范围。} 团队成员可以用模块、目录、分支、作者或时间窗口描述本人的分析范围。系统不按提交数量评价贡献，也不把团队仓库整体自动归为某个学生的个人成果。
  \item \textbf{区分工作产物与公开材料。} 将 \texttt{.learntrace/} 和临时会话导出加入忽略规则。机器审计档案包含更完整的来源索引，应保留在本地；对外仅分享学生检查过的脱敏阅读档案。
\end{enumerate}

上述事项是提高证据质量的建议，不是使用门槛。材料不齐时，系统仍会在授权范围内利用已有 Git、文档和测试日志生成档案，并明确列出缺失信息。学生不应为追求完整叙事补造日志、提交或反思，教师也不应把材料数量直接当作学习质量或个人贡献的评分。
...
最近一次公开持续集成基线为 Git 提交 \texttt{0af1135}。2026 年 9 月 6 日的 GitHub Actions 记录显示，Python 全量测试在 Ubuntu 和 Windows 上均为 \textbf{690 项通过、1 项跳过}，Web 测试为 6 项通过，Agent 测试为 81 项通过；Ruff、格式检查、Pyright、前后端构建、资源同步和 wheel 构建也均通过，而不是仅以“成功收集测试”作为验收依据。以下分类描述测试体系：
...
GitHub Actions 在 Ubuntu 和 Windows 上执行 Ruff、格式检查、Pyright、Pytest、前后端类型检查与构建、生成资源同步以及 wheel 安装验证。上述基线的各项任务均通过，记录地址为 \url{https://github.com/constantinmay/learntrace-skill/actions/runs/34011233296}。双平台任务用于覆盖路径格式、文件锁、子进程和打包行为差异。公开测试样例均经过脱敏，不包含真实学生会话、密钥和私人项目材料。这些结果说明对应版本满足既定工程检查，但不能替代对真实教学价值的案例观察和用户评价。
