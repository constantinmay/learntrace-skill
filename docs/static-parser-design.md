...
1. `discover_static_materials(root)` 遍历可见文件并返回候选文档、日志和项目清单；此阶段不读取候选文件内容。
2. 宿主展示候选范围，由用户确认或删减。
...
- 每个新增、修改、删除、重命名或复制文件产生一条文件级事实，记录增删行数；二进制文件明确标记行数不可用。
- 重命名事件同时引用新旧路径；所有 Git 事实都引用完整提交哈希。
- 只调用不会执行 hook 的 Git 读取子命令，并设置 15 秒超时。

### Markdown/TXT

- 仅接受不超过 1 MiB 的 UTF-8 `.md` 和 `.txt`。
- Markdown 按标题分节，再以段落或围栏代码块为不可拆事实块组合成约 600 字符的事件。
- TXT 按段落生成事件。
- 不以省略号截断正文；较长章节拆成多条带准确行号的事件，从而保留全部可解析文本。

### 已有测试日志

- 支持 pytest 计数、耗时、`FAILED`/`ERROR` 用例，以及 Maven/JUnit 常见的 `Tests run` 汇总行。
...
- 路径/文件：`invalid_source`、`unsupported_document`、`file_too_large`、`invalid_utf8`、`read_error`、`empty_document`、`empty_test_log`；
