...
项目的主要难点集中在三处。第一，Git、文档、测试日志和不同 Agent 会话格式差异很大，系统使用版本化 Schema 将它们统一为带来源引用的事件。第二，完整材料与隐私、上下文规模存在冲突，因此采用轻量索引、按需回读、流式解析和显式截断警告。第三，自然语言报告容易把合理猜测写成事实，LearnTrace 通过事实、候选和学生确认分层，并以结构校验、引用一致性检查和人工复核降低无依据叙述的风险；这些机制不能替代对自然语言语义的最终核查。

\section{作品创新点}

\begin{enumerate}
  \item \textbf{证据分层与学生确认。} 系统把事实、候选推断和学生陈述分开保存，不替学生判断已经学会了什么。
  \item \textbf{多来源协作过程重建。} Git、文档、测试日志和三类编码 Agent 轨迹被统一为可检索事件，同时避免仅凭时间邻近制造因果关系。
  \item \textbf{可回查的学习档案闭环。} 从授权、取证、追问到报告生成，关键内容保留来源；阅读档案与机器审计记录分层，兼顾可读性和复核需要。
\end{enumerate}
...
最近一次公开持续集成基线为 Git 提交 \texttt{8283589}。2026 年 9 月 6 日的 GitHub Actions 运行记录显示，Python 测试在 Ubuntu 和 Windows 上均为 \textbf{686 项通过、1 项跳过}，Ruff、Pyright、前端与 Agent 测试及构建、资源同步和 wheel 安装检查同时通过。运行记录：\url{https://github.com/constantinmay/learntrace-skill/actions/runs/34005218350}。本次材料定稿后的工作树又在 Windows 本地完成全量复测，结果为 \textbf{687 项通过、4 项因本机符号链接或环境条件跳过、0 项失败}，Ruff 与 Pyright 同样通过。公开基线与本地复测采用不同环境，不混写为同一份 CI 结果；这些工程检查也不替代对教学价值的案例观察和用户评价。

\section{快速体验与使用边界}

核心 CLI 需要 Python 3.11 或更高版本；使用自带 Web 智能体工作区还需要 Node.js 22.22.2 或更高版本。前端、Agent 服务和 Skill 均随 Python 包分发，最短启动流程为：
...
添加 AI 会话轨迹时必须逐份授权；LearnTrace 不主动扫描 Agent 历史目录，也不自动运行目标项目测试。机器审计档案可能包含本地来源索引，对外分享前应检查主档案，不应直接上传原始会话或未经检查的机器归档。当前版本主要支持 Git、Markdown/纯文本、常见测试日志和三类编码 Agent 会话格式；候选学习节点不是自动评分，最终叙述仍需学生确认和复核。
