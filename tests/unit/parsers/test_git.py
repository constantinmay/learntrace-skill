...
def test_reports_merge_changes_relative_to_first_parent(tmp_path: Path) -> None:
    ...
    history_index = repository / ".learntrace/evidence/git/history.jsonl"
    ...
    assert details["history_index_path"] == ".learntrace/evidence/git/history.jsonl"
    assert details["history_index_status"] == "requires_generation"
    assert details["history_index_command"] == "learntrace git-index <project>"
    assert details["history_index_expected_head"] == _head(repository)
    assert "was not generated by this parse" in result.warnings[0].message
    assert not history_index.exists()
    assert len(placeholder.source_refs) == 2
    assert result.to_dict()["warnings"][0]["details"]["omitted_side_branch"] == 1

    omitted_commit_id = placeholder.source_refs[0].ref
    index_result = write_git_history_index(repository)
    assert index_result.output_path == history_index
    records = [json.loads(line) for line in history_index.read_text(encoding="utf-8").splitlines()]
    assert records[0]["head"] == details["history_index_expected_head"]
    assert any(record.get("commit_id") == omitted_commit_id for record in records[1:])
