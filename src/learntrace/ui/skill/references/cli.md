...
### `learntrace ui [project_dir] [options]`

Opens the local LearnTrace Agent workspace (web UI) on the loopback
interface and opens the project in a browser. This is the human-facing
workspace entry point; the analysis subcommands above remain the same CLI
that the workspace drives.

| flag | meaning |
|---|---|
| `project_dir` | project to open (default: current directory) |
| `--host H` | loopback bind address only (default `127.0.0.1`) |
| `--port N` | local port; `0` selects a free port |
