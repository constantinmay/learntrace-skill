    ...
    *,
    ...
    for line_number, record in iter_jsonl_records(
        ...
        for pending_call, is_error in _matched_tool_results(
            ...
            events.append(event)
            safe_tool = _safe_tool_name(pending_call.tool)
            raw_paths: tuple[str, ...] = ()
            command: str | None = None
            if safe_tool == "bash":
                command_value = pending_call.input_data.get("command")
                command = command_value if isinstance(command_value, str) else None
            elif safe_tool in _FILE_TOOLS:
                path_value = pending_call.input_data.get("file_path")
                if not isinstance(path_value, str):
                    path_value = pending_call.input_data.get("path")
                if isinstance(path_value, str):
    ...
    for export_index, export_path in enumerate(export_paths):
        ...
        for item in result.trace_metadata:
            ...
            if existing_metadata is not None and existing_metadata != item:
                raise UnsupportedTraceFormatError(
                    "多个 Claude Code 会话包含 ID 相同但元数据冲突的工具记录。"
                )
