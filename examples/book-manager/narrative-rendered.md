# 项目复盘:读书/资源管理系统课程项目(未确认版，未经本人复核)

| 项目 | 证据窗口 | 版本状态 | 复盘提示 | 证据缺口 |
|---|---|---|---|---|
| 读书/资源管理系统课程项目 | 2026-08-10 项目初始化至 2026-08-10 文档补全 | 未确认版,未经本人复核 | 0 条可选(见 `.learntrace/learning-questions.md`) | 0 处 |
...
- **新建书籍时间字段修复** <sub>推断 · 已被本人确认</sub>:学生在确认中说明，修复前曾手动复现新增书籍接口返回 500 的问题，原因是 created_at 字段缺少默认值；随后给 model.go 中的 CreatedAt 添加了 time.Now() 默认值，并再次调用接口验证返回 200 与正确时间戳。[^c-3eac321]

> 系统推断需本人确认或补充;仅据此不能断言因果关系,以本人陈述为准。

## AI 协作

- **可见性地图**:学生授权了一份 OpenCode 会话导出，采用最小保留级别：仅保留时间、工具类型、规范化相对路径、命令摘要与来源宿主。
- **活动形状**:授权轨迹覆盖项目初始化和前后端文件写入阶段，未包含完整会话正文。
- **文件焦点**:轨迹中可见 bash、write 与 edit 工具操作集中在项目根目录、backend/ 与 frontend/src/。
- **关键插曲**：
  1. 最小保留记录显示，OpenCode 在会话早期执行了 git init、mkdir 等 bash 命令，并写入了 README.md 与 .gitignore，对应项目仓库和目录的建立。[^t-595e56e][^t-6cbecaa][^t-5888759]
  2. 最小保留记录显示，后端开发阶段写入了 backend/model.go，并编辑了 backend/handlers.go，对应数据模型定义与 REST 接口实现。[^t-ed171df][^t-b729286]
  3. 最小保留记录显示，前端开发阶段写入了 frontend/src/types.ts 等类型定义文件，并后续编辑了 App.tsx、BookList.tsx 等组件文件。[^t-7480430]
